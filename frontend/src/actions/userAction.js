...
import { 
    ...
    LOAD_USER_FAIL, LOGOUT_SUCCESS, LOGOUT_FAIL, CLEAR_ERRORS, 
    UPDATE_PROFILE_REQUEST,
    UPDATE_PROFILE_SUCCESS,
    UPDATE_PROFILE_FAIL,UPDATE_PASSWORD_SUCCESS,
    ...
    NEW_PASSWORD_FAIL} from "../constants/userConstant"
...
export const login = (email,password) => async(dispatch)=>{
    try{
            ...
            const {data} = await axios.post(
                `/api/v1/users/login`,{email,password},config
            );
...
export const register = (userData) => async(dispatch)=>{
    try{
        ...
        const {data} = await axios.post(
            `/api/v1/users/signup`,userData,config
        );
        dispatch({
...
export const loadUser = ()=> async(dispatch)=>{
        try{
                ...
                const {data} = await axios.get(`/api/v1/users/me`);  //change
                    dispatch({
...
export const logout = () =>async(dispatch)=>{
    try{
         await axios.get(`/api/v1/users/logout`);    //change
        dispatch({
            type:LOGOUT_SUCCESS,
        });
...
export const updatePassword = (passwords) =>async(dispatch)=>{
        ...
        try{
             ...
             const {data} = await axios.put("/api/v1/users/password",passwords,config);
             ...
             dispatch({
                ...
                dispatch({
                    ...
                    payloas:error.response.data.message,
                });
        }
